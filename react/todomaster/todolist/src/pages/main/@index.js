...
const MainPage = () => {
  ...
  const navigateToTodoPage = () => {
    //window.location.href = "/todo"; x 쓰지마 왜? 번들 다 다운로드 돼서 필요없는 리소스를 다 다운해버림
    navigate("/todo"); // o 이거써
  };
  ...
  return (
    ...
    >
      ...
      */}
      {/* <a href="/todo">
        <button>a TodoPage로 이동</button>
      </a>
      <Link to="/todo">
        <button>Link TodoPage로 이동</button>
      </Link> */}
      <button onClick={navigateToTodoPage}>이동</button>
    </div>
  );
};
